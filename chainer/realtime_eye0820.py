...
class CNN(Chain):
    def __init__(self):
        ...
        with self.init_scope():
            self.conv1 = L.Convolution2D(None, 50, 5)
...
load_npz('eye0820.npz', model, strict=True)  # Accuracy87%

def predict(model, x_data):
    x = Variable(x_data.astype(np.float32))
    y = model.predictor(x)
    return np.max(y.data, axis = 1)
...
if __name__ == '__main__':
    ...
    while True:
        ...
        print(np.array(confidences,dtype=np.float32))
